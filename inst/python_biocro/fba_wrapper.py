...
def run_BioCro_FBA(start_afresh=0,save_tag='',weather_file="./../yamls/parameter_files/soybean_weather_2002growingseason.txt",kOrgan_vals_path="./../BioCro3_output_complete.csv",initial_state="./../yamls/parameter_files/soybean_initial_state_fba.txt"):
    if start_afresh:
    ...
    if not os.path.isfile("./../yamls/tempInitialState.txt"):
        copyfile(initial_state,"./../yamls/tempInitialState.txt")
     # If there is no file with carbon allocation values (kOrgan), generate one with BioCro
    if not os.path.isfile(kOrgan_vals_path):
        temp = subprocess.call(['Rscript','../Gro_wrapper_fba_new.R','../yamls/tempInitialState.txt','../yamls/parameter_files/soybean_parameters_biocro3_fba.txt',weather_file,'../yamls/parameter_files/soybean_ss_modules_fba.txt','../yamls/parameter_files/soybean_deriv_modules_fba.txt'])
        temp = subprocess.call(['mv','biocro_output2.txt',kOrgan_vals_path])
    # Load weather file
